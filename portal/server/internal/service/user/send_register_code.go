...
// SendRegisterCode 用户注册发送短信验证码
func (s *Service) SendRegisterCode(ctx context.Context, phoneNumber string) (err error) {
	// 检查account是否已经注册
	...
	if exists {
		log.WithContext(ctx).Info("账号已经已经注册", zap.String("account", phoneNumber))
		return errorx.AccountRegisteredError
	}
	...
	// 保存短信验证码到缓存
	err = s.Cache.SetRegisterCode(ctx, phoneNumber, code)
	if err != nil {
