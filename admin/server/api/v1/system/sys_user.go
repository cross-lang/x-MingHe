...
func (b *BaseApi) Register(c *gin.Context) {
	...
	if len(authorities) > 0 {
		authority, err := authorityService.GetAuthorityInfo(authorities[0])
		var pinfo = make([]map[string]interface{}, 0, 1)
		err = json.Unmarshal([]byte(authority.PInfo), &pinfo)
		if err != nil {
			response.FailWithMessage(err.Error(), c)
			return
		}
		user.Pid = int(pinfo[0]["id"].(float64))
	}

	userReturn, err := userService.Register(*user)
	if err != nil {
		var message = "注册失败"
		if msg := err.Error(); msg == "用户名重复" || msg == "手机号重复" {
			message = msg
		}
		global.GVA_LOG.Error("注册失败!", zap.Error(err))
		response.FailWithDetailed(systemRes.SysUserResponse{User: userReturn}, message, c)
		return
	}
...
// @Security  ApiKeyAuth
// @Produce  application/json
// @Param     data  body      systemReq.ChangeParkReq    true  "新园区id"
// @Success   200   {object}  response.Response{msg=string}  "用户修改当前园区
// @Router    /user/changePark [put]
func (b *BaseApi) ChangePark(c *gin.Context) {
	var req systemReq.ChangeParkReq
	err := c.ShouldBindJSON(&req)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	err = utils.Verify(req, utils.SetUserParkVerify)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	uid := utils.GetUserID(c)
	u := &system.SysUser{GVA_MODEL: global.GVA_MODEL{ID: uid}, Pid: req.Pid}
	err = userService.ChangePark(u)
	if err != nil {
		global.GVA_LOG.Error("修改失败!", zap.Error(err))
		response.FailWithMessage("修改失败，原密码与当前账户不符", c)
		return
