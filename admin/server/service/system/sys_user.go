...
func (userService *UserService) Register(u system.SysUser) (userInter system.SysUser, err error) {
	...
	if !errors.Is(global.GVA_DB.Where("username = ?", u.Username).First(&user).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
		return userInter, errors.New("用户名重复")
	}
	...
	u.UUID = uuid.New()
	u.Authority.PInfo = "{}"
	for k, _ := range u.Authorities {
		u.Authorities[k].PInfo = "{}"
	}
	//默认园区
...
func (userService *UserService) SetUserInfo(req system.SysUser) error {
	var user *system.SysUser
	if req.Phone != "" && !errors.Is(global.GVA_DB.Where("phone = ? and id <> ?", req.Phone, req.ID).First(&user).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
...
func (userService *UserService) ChangePark(u *system.SysUser) (err error) {

	err = global.GVA_DB.Model(&u).Update("p_id", &u.Pid).Error
	return err
}
