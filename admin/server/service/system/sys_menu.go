...
func (menuService *MenuService) GetMenuTree(userId uint) (menus []system.SysMenu, err error) {
	var authorityIds = make([]uint, 0)

	var user *system.SysUser
	err = global.GVA_DB.Model(&system.SysUser{}).Where("id = ?", userId).First(&user).Error
	if err != nil {
		return
	}
	var authorities []system.SysAuthority
	var userAuthority *system.SysUserAuthority
	err = global.GVA_DB.Model(&system.SysUserAuthority{}).Select("sys_user_id").Where("sys_authority_authority_id = 888 and sys_user_id = ?", userId).First(&userAuthority).Error

	if userAuthority.SysUserId > 0 {
		err = global.GVA_DB.Find(&authorities).Error
	} else {
		err = global.GVA_DB.Where("JSON_CONTAINS(p_info, JSON_OBJECT('id', ?))", user.Pid).Find(&authorities).Error
	}
	if err != nil {
		return
	}
	var userAuthorityIds []system.SysUserAuthority
	err = global.GVA_DB.Model(&system.SysUserAuthority{}).Where("sys_user_id = ?", user.ID).Find(&userAuthorityIds).Error
	if err != nil {
		return
	}
	var userAuthorityMap = make(map[uint]uint)
	for _, v := range userAuthorityIds {
		userAuthorityMap[v.SysAuthorityAuthorityId] = 1
	}
	for _, v := range authorities {
		//过滤角色权限
		if userAuthorityMap[v.AuthorityId] == 1 {
			authorityIds = append(authorityIds, v.AuthorityId)
		}

	}
	var subMenus []system.SysMenu
	var subMenuMap = make(map[uint]int)
	for _, v := range authorityIds {
		menuTree, menuErr := menuService.getMenuTreeMap(uint(v))
		if menuErr != nil {
			menuErr = errors.New(menuErr.Error())
			return
		}
		subMenus = menuTree[0]
		for i := 0; i < len(subMenus); i++ {
			err = menuService.getChildrenList(&subMenus[i], menuTree)
			if subMenuMap[subMenus[i].MenuId] == 0 {
				menus = append(menus, subMenus[i])
				subMenuMap[subMenus[i].MenuId] = 1 //第一级去重
			} else {
				if len(subMenus[i].Children) > 0 {
					for k, v := range menus {
						if subMenus[i].MenuId == v.MenuId {
							//第二级菜单去重
							for _, subMenu := range subMenus[i].Children {
								exist := false
								for _, v := range menus[k].Children {
									if v.MenuId == subMenu.MenuId {
										exist = true
									}
								}
								if !exist {
									menus[k].Children = append(menus[k].Children, subMenu)
								}
							}
...
func (menuService *MenuService) GetMenuAuthority(info *request.GetAuthorityId, userId uint) (menus []system.SysMenu, err error) {
	var baseMenu []system.SysBaseMenu
	var SysAuthorityMenus []system.SysAuthorityMenu

	err = global.GVA_DB.Where("sys_authority_authority_id  in (?)", info.AuthorityId).Find(&SysAuthorityMenus).Error
	if err != nil {
		return
	}

	var MenuIds []string
	var authorityIds = make([]string, len(SysAuthorityMenus))
	for i := range SysAuthorityMenus {
		MenuIds = append(MenuIds, SysAuthorityMenus[i].MenuId)
		authorityIds = append(authorityIds, SysAuthorityMenus[i].AuthorityId)
	}
