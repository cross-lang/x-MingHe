...
var ErrRoleNameExistence = errors.New("存在相同角色")
...
func (authorityService *AuthorityService) CreateAuthority(auth system.SysAuthority) (authority system.SysAuthority, err error) {
	...
	if err = global.GVA_DB.Where("authority_name = ?", auth.AuthorityName).First(&system.SysAuthority{}).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		return auth, ErrRoleNameExistence
	}
...
func (authorityService *AuthorityService) CopyAuthority(adminAuthorityID uint, copyInfo response.SysAuthorityCopyResponse) (authority system.SysAuthority, err error) {
	...
	if err = global.GVA_DB.Where("authority_name = ? and authority_id <> ?", copyInfo.Authority.AuthorityName, copyInfo.Authority.AuthorityId).First(&system.SysAuthority{}).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		return authority, ErrRoleNameExistence
	}
	copyInfo.Authority.Children = []system.SysAuthority{}
	menus, err := MenuServiceApp.GetMenuAuthority(&request.GetAuthorityId{AuthorityId: copyInfo.OldAuthorityId}, 0)
	if err != nil {
		return
	}
	...
	if err != nil {
		var delAuthority = system.DelSysAuthority{
			AuthorityId:     copyInfo.Authority.AuthorityId,
			ParentId:        copyInfo.Authority.ParentId,
			DataAuthorityId: copyInfo.Authority.DataAuthorityId,
			Children:        copyInfo.Authority.Children,
			SysBaseMenus:    copyInfo.Authority.SysBaseMenus,
			Users:           copyInfo.Authority.Users,
			DefaultRouter:   copyInfo.Authority.DefaultRouter,
			PInfo:           copyInfo.Authority.PInfo,
		}
		_ = authorityService.DeleteAuthority(&delAuthority)
	}
...
func (authorityService *AuthorityService) UpdateAuthority(auth system.SysAuthority) (authority system.SysAuthority, err error) {
	...
	if err = global.GVA_DB.Where("authority_name = ? and authority_id <> ?", auth.AuthorityName, auth.AuthorityId).First(&system.SysAuthority{}).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		return authority, ErrRoleNameExistence
	}
...
func (authorityService *AuthorityService) DeleteAuthority(auth *system.DelSysAuthority) error {
