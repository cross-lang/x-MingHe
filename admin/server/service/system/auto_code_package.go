...
func (s *autoCodePackage) All(ctx context.Context) (entities []model.SysAutoCodePackage, err error) {
	...
	for i := 0; i < len(serverDir); i++ {
		if serverDir[i].IsDir() {
			serverPackage := model.SysAutoCodePackage{
				...
				Desc:        "系统自动读取/service/" + serverDir[i].Name(),
	...
	for i := 0; i < len(pluginDir); i++ {
		if pluginDir[i].IsDir() {
			...
			if len(dirNameMap) == 0 {
				// 完全符合标准结构
				desc = "系统自动读取/plugin/" + pluginDir[i].Name() + "插件，使用前请确认是否为v2版本插件"
			} else {
				// 缺少某些结构，生成警告描述
				...
				desc = fmt.Sprintf("系统自动读取/plugin/"+pluginDir[i].Name()+"，但是缺少 %s 结构，不建议自动化和mcp使用", strings.Join(missingDirs, "、"))
			}
